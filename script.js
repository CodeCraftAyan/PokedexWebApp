const pokemonFetchData = async (query) => {
  ...
  try {
      ...
      const types = data.types.map(type => `<span class="text-white w-20 text-center px-2 py-1 rounded-lg" style="background-color: ${getTypeColor(type.type.name)}">${type.type.name.toUpperCase()}</span>`).join('');
